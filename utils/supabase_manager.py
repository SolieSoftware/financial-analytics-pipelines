...
class SupabaseManager:
    ...
    def validate_rsi_data(self, data: List[RSIAnalysisResult]) -> bool:
        ...
        required_fields = ['symbol', 'date', 'rsi_value', 'open', 'close', 'high', 'low', 'volume']
        for item in data:
            if not all(hasattr(item, field) for field in required_fields):
                self.logger.error(f"Missing required fields for RSI data: {item}")
                return False
        return True
    
    def batch_insert_rsi_calculations(self, rsi_data_list: List[RSIAnalysisResult]) -> Dict[str, Any]:
        ...
        try: 
            data_list = [
                {
                    ...
                    'volume': int(rsi_data.volume) if rsi_data.volume is not None else None,
                    'created_at': rsi_data.created_at.isoformat() if rsi_data.created_at is not None else datetime.now(timezone.utc).isoformat()
                }
            ...
            response = self.client.table(self.rsi_table_name).insert(data_list).execute()
